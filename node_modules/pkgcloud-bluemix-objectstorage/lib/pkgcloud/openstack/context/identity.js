...
Identity.prototype.authorize = function (options, callback) {
  ...
  authenticationOptions.body = {auth: self.options.auth};
  authenticationOptions.json = true;
  if (self.options.auth && self.options.auth.forceUri){
    authenticationOptions.uri = self.options.auth.forceUri;
  }
  ...
  request(authenticationOptions, function (err, response, body) {
    ...
    else if ( (self.options.auth && !self.options.auth.forceUri) && !body.access.token.tenant) {
      getTenantId(urlJoin(options.url || self.options.url, '/v2.0/tenants'), body.access.token.id);
    }
    else {
      try {
        self._parseIdentityResponse(body, response.headers['x-subject-token']);
        callback();
      }
      catch (e) {
        callback(e);
 ...
 * @private
 */
Identity.prototype._parseIdentityResponse = function (data, idToken) {
  var self = this;
  ...
  if (data.token) {
    ...
    self.token.expires = new Date(self.token.expires_at);
    self.token.id = idToken;
  }

  if (self.useServiceCatalog && data.token.catalog) {
...
Identity.prototype.getServiceEndpointUrl = function (options) {
  //console.log(this);
  options.interfaceName = this.options.auth.interfaceName;
